...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError("Type error: passing {0} to {1}".format(str(args), str(procedure)))
...
class Frame:
    ...
    def rebind(self, sym, val):
        """Rebind the symbol to the new value in the first frame that the symbol is found"""
        if symbol in self.bindings:
            self.bindings[sym] = val
        elif self.parent is not None:
            Frame.rebind(self.parent, sym, val)
        else:
            raise SchemeError("unknown identifier: {0}".format(str(symbol)))
